...
function SignIn() {
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      // Handle the case when the user rejects the request
      if (error.message.includes("User rejected the request.")) {
        // Display a message to the user indicating that the authentication process was canceled
        console.log("User rejected the request. Please try again.");
        alert("User rejected the request. Please try again.");
      } else {
  ...
  return (
    <div className="grid place-items-center h-screen bg-teal-600">
      <button
        onClick={handleAuth}
        type="button"
        ...
        style={{
          ...
          padding: "15px 20px",
          // Increase font size to make the text larger
          fontSize: "20px",
          width: "fit-content", // Adjusted the button width to fit the content
          whiteSpace: "nowrap", // Prevents text from wrapping to the next line
        }}
      >
        <svg
          aria-hidden="true"
          className="w-10 h-8 mr-2 -ml-1"
          viewBox="0 0 2405 2501"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          style={{
            // Scale up the icon by adjusting its width and height
            width: "30px",
            height: "25px",
          }}
        >
